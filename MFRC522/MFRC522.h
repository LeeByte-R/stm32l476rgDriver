...
// SS is PC4, RST PC5
#define MFRC522_SS_0 HAL_GPIO_WritePin(MFRC522_SS_GPIO_Port, MFRC522_SS_Pin, GPIO_PIN_RESET)
#define MFRC522_SS_1 HAL_GPIO_WritePin(MFRC522_SS_GPIO_Port, MFRC522_SS_Pin, GPIO_PIN_SET)
...
extern SPI_HandleTypeDef hspi1;    // MFRC522 accept upto 10MHz, set to 4MHz.
...
void PCD_WriteArrayToRegister(PCD_Register reg, uint8_t count, uint8_t *values);
uint8_t PCD_ReadRegister(PCD_Register reg);
void PCD_ReadArrayFromRegister(PCD_Register reg, uint8_t count, uint8_t *values, uint8_t rxAlign);
...
void GetStatusCodeName(StatusCode code, uint8_t *s,uint32_t *size);
PICC_Type PICC_GetType(uint8_t sak);
void PICC_GetTypeName(PICC_Type piccType,  uint8_t  *s, uint32_t *size);
